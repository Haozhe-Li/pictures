...
export function UploadModal({ isOpen, onClose, initialFiles }: UploadModalProps) {
  ...
  const handleAutoAnnotate = async (item: UploadItem) => {
    if (!item?.file || item.isAnnotating) return

    updateItem(item.id, { isAnnotating: true })
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Auto annotation failed")
      }
      ...
      updateItem(item.id, {
        isAnnotating: false,
        error: err instanceof Error ? err.message : "Auto annotation failed",
      })
    }
  }
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-5xl h-[85vh] flex flex-col bg-white border border-gray-200 rounded-2xl shadow-xl z-50 overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="w-80 border-r border-gray-200 flex flex-col bg-gray-50">
                {/* Authentication */}
                <div className="p-4 space-y-3 border-b border-gray-200 shrink-0">
                  <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Authentication</h3>
                ...
                <div className="flex-1 overflow-hidden flex flex-col min-h-0">
                  <div className="flex items-center justify-between p-4 pb-2">
                    <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Images ({items.length})</h3>
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  <ScrollArea className="flex-1 h-full px-2">
                    <div className="space-y-1 p-2">
                      {items.map((item) => (
                        ...
                        >
                          <div className="w-12 h-12 rounded overflow-hidden bg-white shrink-0 border border-gray-200">
                            <img src={item.previewUrl} className="w-full h-full object-cover" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={cn("text-sm font-medium truncate", !item.title && "text-gray-400 italic")}>
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 p-1 hover:text-red-600 text-gray-400 transition-opacity absolute right-2 top-1/2 -translate-y-1/2"
                          >
              ...
              >
                {isDragging && (
                  <div className="absolute inset-0 z-20 bg-gray-50/80 border-2 border-dashed border-gray-300 rounded-2xl flex items-center justify-center pointer-events-none">
                ...
                {currentItem ? (
                  <div className="p-8 max-w-3xl mx-auto w-full space-y-8">
                    ...
                    <div className="aspect-video bg-gray-50 rounded-xl border border-gray-200 overflow-hidden flex items-center justify-center relative group">
                      ...
                      <div className="absolute top-4 right-4 bg-white px-3 py-1 rounded-full text-xs text-gray-600 border border-gray-200">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm text-gray-600">Auto-generate title & description</h3>
                      <Button
                        onClick={() => handleAutoAnnotate(currentItem)}
                        disabled={currentItem.isAnnotating}
                        className="bg-gray-100 text-gray-900 hover:bg-gray-200"
                        size="sm"
                      >
                        {currentItem.isAnnotating ? (
                          <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <Label className="text-gray-600">Taken Time</Label>
                          <Input
                        ...
                        <div className="space-y-2">
                          <Label className="text-gray-600">Camera Info</Label>
                          <Input
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-4">
                    <div
                      onClick={() => fileInputRef.current?.click()}
                      className="w-full max-w-sm aspect-square border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center gap-4 cursor-pointer hover:border-gray-300 hover:bg-gray-50 transition-all group"
                    >
                      <div className="w-16 h-16 rounded-full bg-white border border-gray-200 flex items-center justify-center group-hover:scale-110 transition-transform">
                        <Upload className="w-8 h-8 text-gray-400" />
                      </div>
                      <div className="space-y-1">
                        <p className="font-medium text-gray-700">Click or drag to upload images</p>
            ...
            <div className="p-6 border-t border-gray-200 bg-gray-50 flex justify-between items-center shrink-0">
              ...
              <div className="flex items-center gap-3">
                <Button variant="ghost" onClick={onClose} className="hover:bg-gray-100 text-gray-700">
                  Cancel
                </Button>
                <Button
